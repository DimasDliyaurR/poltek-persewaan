<div>
    ...
    <x-inner-layout>
        ...
        {{-- <select>
        <option onclick="ExportToExcel('xlsx')"> Excel</option>
    </select> --}}
    ...
    <x-inner-layout>
        <x-title-component class="mb-4 flex flex-col sm:text-sm text-lg">
            ...
            Laporan Transaksi Bulan
        </x-title-component>
        <div class="flex flex-col sm:flex-row justify-between mb-5 sm:w-full">
            <form method="get">
                <select name="kategori"
                    class="block sm: w-full mb-5 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-200 dark:placeholder-white dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    onchange="this.form.submit()">
                    <option value="">All</option>
                    <option value="Asrama" {{ request('kategori') == 'Asrama' ? 'selected' : '' }}>Asrama</option>
                    <option value="GedungLap" {{ request('kategori') == 'GedungLap' ? 'selected' : '' }}>Gedung
                    </option>
                    <option value="Layanan" {{ request('kategori') == 'Layanan' ? 'selected' : '' }}>Layanan</option>
                    <option value="Alat Barang" {{ request('kategori') == 'Alat Barang' ? 'selected' : '' }}>Alat
                        Barang
                    </option>
                </select>
            </form>
            ...
            <div class="w-full md:w-1/2 lg:w-1/2">
                ...
                <div class="relative">
                    ...
                    <form>
                        <input type="search" id="search-transaksi" name="search" id="default-search"
                            class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-200 dark:placeholder-white dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Cari disini.." />
                        {{-- <button
                        class="text-white absolute end-2.5 bottom-2.5 bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-blue-800">Search</button> --}}
        ...
        <button id="export" onclick="ExportToExcel('tabel-laporan-transaksi','xlsx')"
            class="p-2 mb-2 bg-black text-white rounded-md text-sm">Export
            Excel</button>
        <div class="relative overflow-x-auto sm:rounded-lg">
            <table id="tabel-laporan-transaksi"
                class="w-full text-sm text-left rtl:text-right text-gray-800 dark:text-gray-400 border-solid">
                <thead class="text-md text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-white">
                    <tr>
                        <th scope="col" class="px-6 py-3">
