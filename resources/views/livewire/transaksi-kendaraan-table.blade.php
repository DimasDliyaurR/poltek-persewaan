                    ...
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">
                            <div class="flex items-center">
                                Tanggal Sewa
                                <button x-on:click="$wire.set('order', 'tk_tanggal_sewa')">
                                    @if ($orderAction == 0 or $order != '' and $order != 'tk_tanggal_sewa')
                        ...
                        <th scope="col" class="px-6 py-3">
                            <div class="flex items-center">
                                Tanggal Pelaksanaan
                                <button x-on:click="$wire.set('order', 'tk_pelaksanaan')">
                                    @if ($orderAction == 0 or $order != '' and $order != 'tk_pelaksanaan')
                                        <svg class="w-3 h-3 ms-1.5" aria-hidden="true"
                        ...
                        <th scope="col" class="px-6 py-3">
                            <div class="flex items-center">
                                Tanggal Kembali
                                <button x-on:click="$wire.set('order', 'tk_tanggal_kembali')">
                                    @if ($orderAction == 0 or $order != '' and $order != 'tk_tanggal_kembali')
                                        <svg class="w-3 h-3 ms-1.5" aria-hidden="true"
                                            xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                            viewBox="0 0 24 24">
                                            <path
                        ...
                        <th scope="col" class="px-6 py-3">
                            <div class="flex items-center">
                                Sub Total
                                <button x-on:click="$wire.set('order', 'tk_sub_total')">
                                    @if ($orderAction == 0 or $order != '' and $order != 'tk_sub_total')
                                        <svg class="w-3 h-3 ms-1.5" aria-hidden="true"
                                            xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                            viewBox="0 0 24 24">
                                            <path
                    ...
                    @forelse($transaksiKendaraan as $row)
                        <tr id="parent" onclick="toggleTable(this)" class="cursor-pointer"
                            ...
                            <td>
                                ...
                                <svg id="dropdown-up" xmlns="http://www.w3.org/2000/svg" width="1em"
                                    height="1em" viewBox="0 0 24 24">
                                    <path fill="none" stroke="currentColor" stroke-linecap="round"
                                        stroke-linejoin="round" stroke-width="2" d="m10 17l5-5m0 0l-5-5" />
                                </svg>
                                <svg id="dropdown-down" class="hidden" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <td class="px-6 py-4 dark:text-white">
                                Rp. {{ number_format($row->tk_sub_total, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-4 dark:text-white">
                                {{ $row->status }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('laporan.kendaraan.show', $row->id) }}"
                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Lihat
                                    Detail</a>
                            </td>
                        </tr>
                        <tr class="hidden transition-all duration-300">
                            <td colspan="8" class="px-[4.5rem] py-3 bg-gray-50">
                                <div>
                                    <ul class="flex gap-x-32">
                                        @foreach ($row->kendaraans as $kendaraan)
                                            <li>{{ $kendaraan->merkKendaraan->mk_merk }}</li>
                                            <li>{{ $kendaraan->merkKendaraan->mk_seri }}</li>
                                            <li>Rp.
                                                {{ number_format($kendaraan->merkKendaraan->mk_tarif, 0, ',', '.') }}
                                            </li>
                                            <li>Metode Pembayaran
                                                ({{ $kendaraan->merkKendaraan->paymentMethod->is_dp ? 'DP' : 'LUNAS' }})
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
