...
@section('content')
    ...
    <x-inner-layout>
        ...
        <div class="relative overflow-x-auto sm:rounded-lg">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-800 dark:text-gray-400 border-solid">
                ...
                <tbody>
                    @forelse($propertyGedungLaps as $row)
                        <tr class="bg-white border-b dark:bg-gray-700 dark:border-gray-700 dark:hover:bg-gray-400">
                            <td class="px-6 py-4 dark:text-white">
                                {{ $row->pg_nama }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <x-delete-button action="admin/propertyGedungLap/delete/{{ $row->id }}"
                                    id="{{ $row->id }}" nama="{{ $row->pg_nama }}"></x-delete-button>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <x-inner-layout>
        ...
        <form action="{{ asset('admin/propertyGedungLaps/create') }}" method="post" enctype="multipart/form-data">
            @csrf

            <div class="mb-5">
                <label for="pg_nama" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                    Property</label>
                <input name="pg_nama"
                    class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-4 @error('pg_nama') border-red-500 @enderror"
                    value="{{ old('pg_nama') }}" type="text">
