        ...
        <div class="container px-24 py-24 mx-auto">
            <div class="bg-white w-full xl:w-1/2 md:w-1/2 lg:w-1/2 m-auto h-full xl:p-10 p-5 py-12 ">
                ...
                <div class="py-5 relative">
                    ...
                    <p class="absolute right-0 font-bold">{{ $detailTransaksi[0]->users->profile->nama_lengkap }}</p>
                </div>
                <div class="py-5 relative">
                ...
                <div class="relative overflow-x-auto mt-10">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
                        ...
                        <tbody>
                            @foreach ($detailTransaksi as $item)
                                @foreach ($item->kendaraans as $subItem)
                                    <tr class="bg-white border-b">
                                        ...
                                        <td class="px-6 py-4">
                                            {{ \Carbon\Carbon::parse($item->tk_tanggal_sewa)->isoFormat('D MMMM Y') }}</td>
                                        <td class="px-6 py-4">
                                            {{ \Carbon\Carbon::parse($item->tk_tanggal_kembali)->isoFormat('D MMMM Y') }}
                                        </td>
                                        <td class="px-6 py-4">{{ $item->tk_durasi }}</td>
                                        <td class="px-6 py-4">Rp
                                            {{ number_format($item->tk_sub_total, 0, ',', '.') }}
                                        </td>
                                    </tr>
                                @endforeach
                            @endforeach
                            @if ($totalPromo)
                                <tr>
                                    <td colspan="5" class="px-6 py-1">Diskon </td>
                                    <td class="px-6 py-1">Rp {{ $totalPromo }}</td>
                                </tr>
                            @endif
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center">Sub Total </td>
                ...
                <div class="">
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <p>{{ $detailTransaksi[0]->kendaraans[0]->merkKendaraan->paymentMethod->is_dp ? 'DP' : 'Lunas' }}
                        </p>
                    </div>
                    <div class="flex justify-between mb-2">
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <p>Rp {{ number_format($detailTransaksi[0]->tk_sub_total, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        </div>
