...
class TransaksiGedungLapTable extends Component
{
    ...
    public function render(TransaksiService $transaksiService)
    {
        ...
        if (isset($this->searchAction["search"]) and $this->searchAction["search"] != "")
            $transaksiGedungLap = $transaksiService->searchTransaksiGedungLap($this->searchAction["search"]);
        ...
        if ($this->orderAction != 0) {
            $transaksiGedungLap = $transaksiGedungLap->orderBy($this->order, $sorted);
        }

        $totalTransaksiBelumBayar = 0;
        $totalTransaksiSudahBayar = 0;

        foreach ($transaksiGedungLap->paginate(5) as $row) {
            if ($row->status == "terbayar") {
                $totalTransaksiSudahBayar += $row->tg_sub_total;
            } else if ($row->status == "belum bayar") {
                $totalTransaksiBelumBayar += $row->tg_sub_total;
            }
        }
